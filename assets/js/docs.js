...
async function verifyOtp() {
  ...
  try {
    ...
    showToast(
      "Verification failed — incorrect OTP."
    );
...
/*
   OTP is intentionally NOT saved
   in sessionStorage/localStorage.

   Therefore every page reload
   requires OTP again.
*/

otpGate.classList.remove("hidden");

document.body.classList.add("otp-locked");
    ...
    .forEach(button => {
      ...
      button.addEventListener(
        ...
        event => {
          ...
          filteredDocuments[index].favorite =
            !filteredDocuments[index].favorite;
...
function loadViewer() {
  ...
  const doc =
    filteredDocuments[viewerIndex];

  $("#viewerName").textContent =
    doc.name;

  $("#viewerCategory").textContent =
    `${doc.category} · Document Photo`;

  $("#viewerCounter").textContent =
    `${viewerIndex + 1} / ${filteredDocuments.length}`;
...
$("#viewerImage").addEventListener(
  ...
  () => {
    ...
    $("#viewerLoading").innerHTML =
      "Photo not found — check assets/images/documents/";
